...
public class JinformesAyC extends javax.swing.JFrame {

    ArrayList<Balance> cierreArray, aperturaArray, balTemp;

    DefaultTableModel datosTable;
    Integer ingreso = 0, egreso = 0;

    String tipo = "", fecha, fechaT;
    ...
    public void generarInforme(String condicion1, String condicion2) {
        System.out.println("vista.JinformeCyB.generarInforme()");

        datosTable.setRowCount(0);
        inicio.gAyC.actualizarAyC();
        balTemp = new ArrayList<>();
        aperturaArray = new ArrayList<>();
        cierreArray = new ArrayList<>();
        ...
        aperturaArray = main.balanceDAO.listar(condicion1, "caja");
        cierreArray = main.balanceDAO.listar(condicion2, "caja");
        System.out.println("Sizes Apertura: " + aperturaArray.size() + " | Cierre: " + cierreArray.size());

        boolean isCierre = false;
        boolean isApertura = false;
        boolean isTodos = false;

        if (cbFiltrar.getSelectedItem().equals("Cierre")) {
            isCierre = true;
        } else {
            if (cbFiltrar.getSelectedItem().equals("Apertura")) {
                isApertura = true;
            } else {
                if (cbFiltrar.getSelectedItem().equals("Apertura y Cierre")) {
                    isTodos = true;
                }
            }
        }

        int cierreSize = cierreArray.size();

        for (int i = 0; i < cierreSize; i++) {

            if (isTodos) {
                ingresarTabla(aperturaArray.get(i).getFecha(), aperturaArray.get(i).getConcepto(), aperturaArray.get(i).getIngreso(), aperturaArray.get(i).getEgreso());

                balTemp = main.balanceDAO.listar(" where fecha='" + cierreArray.get(i).getFecha() + "' ", "caja");
                System.out.println("Sizes Elementos del dia: " + balTemp.size());
                for (int j = 0; j < balTemp.size(); j++) {
                    if (!balTemp.get(j).getConcepto().contains("*") && !balTemp.get(j).getConcepto().contains("&")) {
                        ingresarTabla(balTemp.get(j).getFecha(), balTemp.get(j).getConcepto(), balTemp.get(j).getIngreso(), balTemp.get(j).getEgreso());
                    }
                }

                ingresarTabla(cierreArray.get(i).getFecha(), cierreArray.get(i).getConcepto(), cierreArray.get(i).getIngreso(), cierreArray.get(i).getEgreso());
                ingresarTablaVacio();
            } else {
                ...
                try {
                    ...
                    System.out.println("vista.JinformesAyC.generarInforme() error, fuera de rango");
                }
            }
    ...
    private void initComponents() {
        ...
        tableInforme =  new ColorTablas.ColorTablaAyC();
        ...
        jPanel3.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Filtrar", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(255, 255, 255))); // NOI18N

        cbFiltrar.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        cbFiltrar.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Apertura y Cierre", "Apertura", "Cierre" }));
        ...
        jPanel3Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel3Layout.createSequentialGroup()
                ...
                .addGap(18, 18, 18)
                .addComponent(cbFiltrar, 0, 139, Short.MAX_VALUE)
                .addContainerGap())
        );
    ...
    public void comprobarMes() {
        ...
        String condicion1 = "WHERE (concepto LIKE '%Apertura%')  and (fecha BETWEEN '" + year + "/" + mouth + "/01' AND '" + year + "/" + mouth + "/31')  AND idUsuarios='" + getIdUsuario() + "'";
        String condicion2 = "WHERE (concepto LIKE '%Cierre%')  and (fecha BETWEEN '" + year + "/" + mouth + "/01' AND '" + year + "/" + mouth + "/31')  AND idUsuarios='" + getIdUsuario() + "'";

        generarInforme(condicion1, condicion2);
    }
    ...
    public void comprobarRango() {
        ...
        String condicion1 = "WHERE (concepto LIKE '%Apertura%')  and (fecha BETWEEN '" + fechaI + "' AND '" + fechaF + "') AND idUsuarios='" + getIdUsuario() + "'";
        String condicion2 = "WHERE (concepto LIKE '%Cierre%')  and (fecha BETWEEN '" + fechaI + "' AND '" + fechaF + "') AND idUsuarios='" + getIdUsuario() + "'";
    ...
    public void comprobarAno() {
        ...
        String condicion1 = "WHERE (concepto LIKE '%Apertura%')  and (fecha BETWEEN '" + year + "/01/01' AND '" + year + "/12/31') AND idUsuarios='" + getIdUsuario() + "'";
        String condicion2 = "WHERE (concepto LIKE '%Cierre%')  and (fecha BETWEEN '" + year + "/01/01' AND '" + year + "/12/31') AND idUsuarios='" + getIdUsuario() + "'";
    ...
    private void ingresarTabla(String fecha, String concepto, Integer x, Integer y) {
        Object[] fila = new Object[datosTable.getColumnCount()];
        fila[0] = fecha;
        fila[1] = concepto;
        fila[2] = x;
    ...
    private void ingresarTablaVacio() {
        Object[] fila = new Object[datosTable.getColumnCount()];
        fila[0] = "";
        fila[1] = "";
        fila[2] = "";
