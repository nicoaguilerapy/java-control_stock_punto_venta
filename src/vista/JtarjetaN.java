...
public class JtarjetaN extends javax.swing.JFrame {
    ...
    public JtarjetaN() {
        ...
        this.setTitle("Tarjeta -" + inicio.version);
    ...
    public void actualizar() {
        ...
        main.balanceDAO.actualizarTarjeta("" + condicion);
        datosBalance = (DefaultTableModel) tableBalance.getModel();
        datosBalance.setRowCount(0);

        Object[] fila = new Object[datosBalance.getColumnCount()];

        for (int i = 0; i < main.balanceDAO.resultadoTarjeta.size(); i++) {

            Usuarios usuario = (Usuarios) main.usuariosDAO.mapUsuarios.get(main.balanceDAO.resultadoTarjeta.get(i).getIdUsuarios());

            //fecha = main.balanceDAO.resultadoTarjeta.get(i).getFecha();
            //fecha = inicio.gFechas.invertir(fecha);
            sumI = sumI + main.balanceDAO.resultadoTarjeta.get(i).getIngreso();
            sumE = sumE + main.balanceDAO.resultadoTarjeta.get(i).getEgreso();

            fila[0] = main.balanceDAO.resultadoTarjeta.get(i).getId();
            fila[1] = inicio.gFechas.invertir(main.balanceDAO.resultadoTarjeta.get(i).getFecha());
            fila[2] = main.balanceDAO.resultadoTarjeta.get(i).getConcepto();
            fila[3] = main.balanceDAO.resultadoTarjeta.get(i).getIngreso();
            fila[4] = main.balanceDAO.resultadoTarjeta.get(i).getEgreso();
    ...
    private void initComponents() {
        ...
        jLabel8.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/icon-ventas-tarjeta.png"))); // NOI18N
    ...
    private void tableBalanceMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableBalanceMouseClicked
        ...
        for (int i = 0; i < main.balanceDAO.resultadoTarjeta.size(); i++) {
            ...
            if (filaSelecionada == main.balanceDAO.resultadoTarjeta.get(i).getId()) {

                char[] concepto = main.balanceDAO.resultadoTarjeta.get(i).getConcepto().toCharArray();
                if (concepto[0] == '#' || concepto[0] == '*') {
                    ...
                    if (!inicio.login.jMenu.cierreCaja && inicio.login.jMenu.aperturaCaja) {
                        id = main.balanceDAO.resultadoTarjeta.get(i).getId();
                        txtConcepto.setText(main.balanceDAO.resultadoTarjeta.get(i).getConcepto());
                        txtEgreso.setText("" + main.balanceDAO.resultadoTarjeta.get(i).getEgreso());
                        txtIngreso.setText("" + main.balanceDAO.resultadoTarjeta.get(i).getIngreso());
                        try {
                            fecha = inicio.gFechas.invertir(main.balanceDAO.resultadoTarjeta.get(i).getFecha());
                        } catch (Exception e) {
                        }
                        botones(true, true, true);
                    }
