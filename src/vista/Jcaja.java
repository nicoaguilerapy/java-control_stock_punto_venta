...
public class Jcaja extends javax.swing.JFrame {
    ...
    public Jcaja() {
        ...
        this.setTitle("Caja -" + inicio.version);
    ...
    public void actualizar() {
        ...
        main.balanceDAO.actualizarCaja("" + condicion);
        datosBalance = (DefaultTableModel) tableBalance.getModel();
        datosBalance.setRowCount(0);

        Object[] fila = new Object[datosBalance.getColumnCount()];

        for (int i = 0; i < main.balanceDAO.resultadoCaja.size(); i++) {
            if (!main.balanceDAO.resultadoCaja.get(i).getConcepto().contains("&Sin")) {
                Usuarios usuario = (Usuarios) main.usuariosDAO.mapUsuarios.get(main.balanceDAO.resultadoCaja.get(i).getIdUsuarios());

                //fecha = main.balanceDAO.resultadoCaja.get(i).getFecha();
                //fecha = inicio.gFechas.invertir(fecha);
                sumI = sumI + main.balanceDAO.resultadoCaja.get(i).getIngreso();
                sumE = sumE + main.balanceDAO.resultadoCaja.get(i).getEgreso();

                fila[0] = main.balanceDAO.resultadoCaja.get(i).getId();
                fila[1] = inicio.gFechas.invertir(main.balanceDAO.resultadoCaja.get(i).getFecha());
                fila[2] = main.balanceDAO.resultadoCaja.get(i).getConcepto();
                fila[3] = main.balanceDAO.resultadoCaja.get(i).getIngreso();
                fila[4] = main.balanceDAO.resultadoCaja.get(i).getEgreso();
    ...
    private void initComponents() {
        ...
        jLabel8.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/menu-caja.png"))); // NOI18N
    ...
    private void tableBalanceMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableBalanceMouseClicked
        ...
        for (int i = 0; i < main.balanceDAO.resultadoCaja.size(); i++) {
            ...
            if (filaSelecionada == main.balanceDAO.resultadoCaja.get(i).getId()) {

                char[] concepto = main.balanceDAO.resultadoCaja.get(i).getConcepto().toCharArray();
                if (concepto[0] == '#' || concepto[0] == '*') {
                    ...
                    if ((!inicio.login.jMenu.cierreCaja && inicio.login.jMenu.aperturaCaja) || inicio.userObj.getRango().equals("Administrador")) {
                        id = main.balanceDAO.resultadoCaja.get(i).getId();
                        txtConcepto.setText(main.balanceDAO.resultadoCaja.get(i).getConcepto());
                        txtEgreso.setText("" + main.balanceDAO.resultadoCaja.get(i).getEgreso());
                        txtIngreso.setText("" + main.balanceDAO.resultadoCaja.get(i).getIngreso());
                        try {
                            fecha = inicio.gFechas.invertir(main.balanceDAO.resultadoCaja.get(i).getFecha());
                        } catch (Exception e) {
                        }
                        botones(true, true, true);
                    }
